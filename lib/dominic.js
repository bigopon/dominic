(function(root, factory) {
  if (typeof define === 'function' && define.amd) define([], factory); else if (typeof exports === 'object') module.exports = factory(); else root.CreateElement = factory();
})(this, function() {
  var win = window;
  ...
  var evtConfig = [ 'click', 'mousedown', 'mouseup', 'mouseover', 'mouseout', 'mouseenter', 'mouseleave', 'mousemove', 'dragstart', 'dragend', 'drag', 'dragover', 'dragenter', 'dragout', 'drop', 'blur', 'focus', 'keydown', 'keypress', 'keyup', 'change', 'input', 'submit', 'touchstart', 'touchmove', 'touchend', 'wheel', 'scroll' ];
  ...
  var CreateElement = function(name, attrs, root, data) {
    ...
    for (var i = 0, length = attributes.length; i < length; i++) {
      ...
      if (keyIdx == -1) el[key] = val; else {
        if (keyIdx < dimRange) el.style[key] = isNaN(val) ? val : val + 'px'; else if (keyIdx < disRange) el.style[key] = val; else if (keyIdx < shareRange) assign(injectOpts = injectOpts || {}, val); else if (keyIdx < fnRange) (delaySetups = delaySetups || []).push({
          key: key,
          val: val
        }); else if (keyIdx < evtRange) (delayEvts = delayEvts || []).push({
          key: key,
          val: val
        }); else if (keyIdx < cbRange) (delayCb = delayCb || {})[key] = val; else if (keyIdx < propRange) (delayProps = delayProps || {})[key] = val; else if (keyIdx < textRange) el.appendChild(doc.createTextNode(val)); else delayRoot = val;
      }
    }
    if (delayProps) setDelayProps(el, delayProps);
