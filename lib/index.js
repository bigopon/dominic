(function (root, factory) {
	...
	/**
	 * @param nodeList {NodeList} list of nodes to execute
	...
	/**
	 * @param el {Element} element to perform check
	 * @param nodeList {NodeList} list of nodes to perform check on element
	 * @return {Number}
	 */
		...
		defProps(Obs.prototype, {
			add: {
				value: function (root, opts, injectOpts) {
                    ...
                    var defaultOpts = assign({ attrs: { dataTk: thisTplKey } }, injectOpts)
	...
	function getNextDifferentElByKey(startEl, key) {
		while (startEl) {
			if (startEl.getAttribute('data-tk') !== key)
				return startEl
			else
