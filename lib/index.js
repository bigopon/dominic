(function (root, factory) {
	...
	var fakeArray = []
	var fakeOpts = {},
		toString = fakeOpts.__proto__.toString,
		hasOwn = fakeOpts.__proto__.hasOwnProperty
	...
	var areStrOrNum = function (vals) {
		...
		for (var i = 0; i < length; i++)
			count += (isStrOrNum(vals[i])  ? 1 : 0)
		return count === length
	}
	var isFn = function (val) { return typeof val === 'function' }
	var c2d = function (str) { return str.replace(/([A-Z])/g, '-$1').toLowerCase() }
	...
	function queryAllDirect(root, key, value) {
		...
		for (var i = 0; i < length; i++) {
			var child = childNodes[i]
			if (hasOwnProperty.call(child, key) && child[key] === value)
		...
		defProps(Refs.prototype, {
			...
			removeRef: {
				value: function (refName) {
					if (hasOwnProperty.call(this, refName))
	...
	var Obs = (function () {
		...
		function removeAllEvts(el) {
			if (hasOwnProperty.call(el, 'evts'))
				removeEvt(el)
		...
		function removeRef(el, root) {
			...
			if (elRefType === 'p')
				el.parentNode.refs.removeRef(el.ref)
			else
				root.refs.removeRef(el.ref)
		}
		...
		defProps(Obs.prototype, {
			add: {
				value: function (root, opts, injectOpts) {
					...
					var cacheOpts = assign2({}, opts, { obsProp: obsProp }, 'tplFn,scope,for,root,observeProp,alwaysIterate')
	...
	function for2dom(root, defs, realRoot, injectOpts, start) {
		if (hasOwnProperty.call(defs, 'observeProp')) {
			...
			// Need to do this here. other fns doesn't have chance to assign
			if (isDom(cFromTpl)) {
	...
	function setStyleNoOverride(el, opts) {
		...
		for (var i = 0; i < styles.length; i++) {
			...
			if (pxStyle.indexOf(styleKey) !== -1)
				elStyle[styleKey] = isNaN(styleVal) ? styleVal : (styleVal + 'px')
			else
				elStyle[styleKey] = styleVal
		}
	}
	...
	function assignDefs2Node(node, injectOpts) {
		for (var opt in injectOpts) {
			if (injectOpts.hasOwnProperty(opt)) {
				...
				// else if (pxStyle.indexOf(opt) !== -1 && node.nodeType === Node.ELEMENT_NODE)
				// 	node.style[opt] = 
				else
					node[opt] = val
			}
	...
	function assignDefs2NodeList(nodes, injectOpts) {
		for (var i = 0; i< nodes.length; i++)
			assignDefs2Node(nodes[i], injectOpts)
	}
	...
	function arrV2dom(root, defs, realRoot, injectOpts, start) {
		for (var i = 0; i < defs.length; i++) {
			...
			if (isDom(opts)) {
				...
				if (injectOpts && !isStrOrNum(opts))
					assign3(opts, injectOpts)
				setChildren(root, opts, realRoot, injectOpts, start)
			}
		}
	}
	...
	function getNextDifferentElByKey(startEl, key) {
		if (startEl) {
			if (!hasOwnProperty.call(startEl, '__key') || startEl.__key !== key)
				return startEl
			else
		...
		createElement: {
			value: function(name, opts) {
				...
				if (firstArgType === 'number')
					throw Error('Invalid element tag name')
			...
			value: function(el, opts, thisArg) {
				...
				if (isPlainObject(opts)) {
					...
					for (var i = 0; i < evtTypes.length; i++) {
						...
						evts.push({ key: eType, val: evtOpts })
						setters['setevents'](el, assign({ type: eType }, evtOpts), realThis)
					}
				}
				else
					setters['setevents'](el, opts, realThis)
			}
		},
		/**
		 * Set up window object for usage in non browser environment
